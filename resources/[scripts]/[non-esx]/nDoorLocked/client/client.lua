...
function ToggleDoor()
	...
	local secretCode = KeyboardInput("Code de sécurité :", "", 4)
	...
	for i=1, #Config.DoorList do
		...
		if distance < maxDistance then
			...
			if tonumber(secretCode) == nil then
				ESX.ShowNotification("~y~Entrez un code correct.")
				return false
			end
			...
			if tonumber(secretCode) == doorID.authorizedCodes then
				ESX.ShowNotification("~g~Accès autorisé")
	...
	while true do
		...
		for i=1, #Config.DoorList do
			...
			if distance < maxDistance then
				...
				local displayText = '~g~Ouvert' --Ouvert
				if doorID.locked then
					displayText = '~r~Fermer' --Fermer
				end
