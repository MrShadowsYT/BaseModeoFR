...
function admin_no_clip()
	...
	if noclip then
		...
		TriggerEvent('esx:showAdvancedStreamedNotification', 'RD Leaks', '~b~Administration', 'Vous avez ~g~activé~w~ le NoClip', 'CHAR_SOCIAL_CLUB', 'logo', 0)
	else
		...
		TriggerEvent('esx:showAdvancedStreamedNotification', 'RD Leaks', '~b~Administration', 'Vous avez ~r~désactivé~w~ le NoClip', 'CHAR_SOCIAL_CLUB', 'logo', 0)
	end
end
...
function admin_godmode()
	...
	if godmode then
		SetEntityInvincible(plyPed, true)
		TriggerEvent('esx:showAdvancedStreamedNotification', 'RD Leaks', '~b~Administration', 'Vous avez ~g~activé~w~ le Mode invincible', 'CHAR_SOCIAL_CLUB', 'logo', 0)
	else
		SetEntityInvincible(plyPed, false)
		TriggerEvent('esx:showAdvancedStreamedNotification', 'RD Leaks', '~b~Administration', 'Vous avez ~r~désactivé~w~ le Mode invincible', 'CHAR_SOCIAL_CLUB', 'logo', 0)
	end
end
...
function admin_tp_marker()
	...
	if DoesBlipExist(WaypointHandle) then
		...
		TriggerEvent('esx:showAdvancedStreamedNotification', 'RD Leaks', '~b~Administration', 'Vous vous êtes ~g~téléporté~w~ sur le marqueur', 'CHAR_SOCIAL_CLUB', 'logo', 0)
	else
		TriggerEvent('esx:showAdvancedStreamedNotification', 'RD Leaks', '~b~Administration', 'Aucun ~r~marqueur~w~ sur la carte', 'CHAR_SOCIAL_CLUB', 'logo', 0)
	end
end
...
function AddSubMenuWeaponMenu(menu)
	weaponMenu = _menuPool:AddSubMenu(menu.SubMenu, 'Mes armes')
...
function AddSubMenuActionMenu(menu)
	actionmenu = _menuPool:AddSubMenu(menu.SubMenu, ('Actions civil'))
	actionmenu.SubMenu:SetMenuWidthOffset(20)
	actionmenu.Item:RightLabel("→→→") 

	local PeggyItem = NativeUI.CreateItem(('Porter | Lacher'), "")
	actionmenu.SubMenu:AddItem(PeggyItem)

	local ragdollItem = NativeUI.CreateItem(('Dormir | Se Réveiller'), "")
	actionmenu.SubMenu:AddItem(ragdollItem)

	local HostageItem = NativeUI.CreateItem(('Prendre en otage'), "")
...
function AddSubMenuOptionMenu(menu)
	Option = _menuPool:AddSubMenu(menu.SubMenu, ('Options'))
	Option.SubMenu:SetMenuWidthOffset(20)
	Option.Item:RightLabel("→→→")

	local minimap = NativeUI.CreateCheckboxItem('Afficher | Cacher le GPS', "")
	...
	local sauvegarderperso = NativeUI.CreateItem('Sauvegarder votre Personnage', "")
...
function AddSubMenuFiltreMenu(menu)
	Filtre = _menuPool:AddSubMenu(menu.SubMenu, ('Filtre'))
	Filtre.SubMenu:SetMenuWidthOffset(20)
	Filtre.Item:RightLabel("→→→")
   
local vunormal = NativeUI.CreateItem('Normal', "")
Filtre.SubMenu:AddItem(vunormal)

local vulumiereameliores = NativeUI.CreateItem('Vue & lumières améliorées', "")
Filtre.SubMenu:AddItem(vulumiereameliores)

local couleuramplifies = NativeUI.CreateItem('Couleurs amplifiées', "")
Filtre.SubMenu:AddItem(couleuramplifies)

local noiretblanc = NativeUI.CreateItem('Vu Noir & Blanc', "")
Filtre.SubMenu:AddItem(noiretblanc)

local optimiserfps = NativeUI.CreateItem('Optimisation', "")
...
function AddMenuVetementMenu(menu)
	vetementmenu = _menuPool:AddSubMenu(menu, ('Vêtements'))
...
function AddSubMenuClothesMenu(menu)
	clothesMenu = _menuPool:AddSubMenu(menu.SubMenu, 'Habits')
...
function AddMenuAnimationMenu(menu)
	...
	local emoteItem = NativeUI.CreateItem(('Animations'), '')
