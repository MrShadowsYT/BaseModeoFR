...
local blips = {
	{title="Salle de sport", colour=4, id=311, x = -1201.2257, y = -1568.8670, z = 4.6101}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour louer un ~b~vélo')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Vous avez déjà un ~r~vélo")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le ~w~menu ~b~Crossfit')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour soulever des ~g~Poids')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Préparation de ~g~l'exercice~w~Vous êtes chaud?...")
						Citizen.Wait(1000)					
						...
						elseif membership == false then
							ESX.ShowNotification("Vous avez besoin d'une licence pour faire un ~r~ exercice")
						end
					elseif training == true then
						ESX.ShowNotification("Vous avez besoin de vous reposer...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire ~g~des tractions')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire ~g~des pompes')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire ~g~du yoga')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire ~g~des Abdos')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Allez go !!! on peut reprendre ")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = "Licence a l'heure", value = 'hours'},
				{label = "Licence de sport a l'année", value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Ouvert 7/7 et ~g~24h/24h~w~")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Boutique salle de sport',
            elements = {
				{label = 'protéine (70€)', value = 'protein_shake'},
				{label = 'volvic (30€)', value = 'water'},
				{label = 'Barre Protéiné (70€)', value = 'sportlunch'},
				{label = 'Powerade (50€)', value = 'powerade'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Crossfit : inscription',
            elements = {
				{label = 'Carte de membre (500€)', value = 'membership'},
            }
        },
        function(data, menu)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'bike_menu',
        {
            ...
            elements = {
				...
				{label = 'Cruiser (100€)', value = 'cruiser'},
				{label = 'Fixter (150€)', value = 'fixter'},
				{label = 'Scorcher (200€)', value = 'scorcher'},
            }
        },
        function(data, menu)
