...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      if IsControlJustPressed(1, KeyTakeCall) or IsControlJustPressed(1, 18) then
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
      exports['screenshot-basic']:requestScreenshotUpload(data.url, data.field,{
        headers = {
          ['authorization'] = string.format('Client-ID %s', 'b5501e3602e3238'),
...
if myPhoneNumber ~= 'nil' and myPhoneNumber ~= nil and tonumber(myPhoneNumber) ~= 0 and myPhoneNumber ~= '' then
  ESX.TriggerServerCallback('gcphone:getItemAmount', function(qtty)
    ...
    ESX.ShowAdvancedNotification("LifeInvader", "Télécom", "Vous n'avez pas de ~r~téléphone~s~", "CHAR_CHAT_CALL",9)
    end
  end, 'tel')
else
  ESX.ShowAdvancedNotification("LifeInvader", "Télécom", "Pas de ~r~carte sim lié~s~, aller en acheter une", "CHAR_LIFEINVADER",9)
end
end
