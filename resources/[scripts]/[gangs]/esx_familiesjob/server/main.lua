...
AddEventHandler('esx_families:getStockItem', function(itemName, count)
  ...
  TriggerEvent('esx_addoninventory:getSharedInventory', 'society_families', function(inventory)
    ...
    if item.count >= count then
      ...
      TriggerClientEvent('esx:showNotification', xPlayer.source, _U('quantity_invalfamilies'))
    end
...
ESX.RegisterServerCallback('esx_families:getOtherPlayerData', function(source, cb, target)
  ...
  if Config.EnableESXfamiliesentity then

    local xPlayer = ESX.GetPlayerFromId(target)

    local familiesentifier = GetPlayerfamiliesentifiers(target)[1]

    local result = MySQL.Sync.fetchAll("SELECT * FROM users WHERE familiesentifier = @familiesentifier", {
      ['@familiesentifier'] = familiesentifier
    })
  ...
  if Config.EnableESXfamiliesentity then
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        local foundfamiliesentifier = nil
        ...
        for i=1, #result, 1 do
          ...
          if vehicleData.plate == plate then
            foundfamiliesentifier = result[i].owner
            break
          end

        end

        if foundfamiliesentifier ~= nil then

          MySQL.Async.fetchAll(
            'SELECT * FROM users WHERE familiesentifier = @familiesentifier',
            {
              ['@familiesentifier'] = foundfamiliesentifier
            },
            function(result)
